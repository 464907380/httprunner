...
def customer_pager(base_url, current_page, total_page, cnt):
    """
    返回可分页的html
    ...
    pager_list = []
    count = "<li><a href=''>共%d项</a></li>" % (cnt)
    pager_list.append(count)
...
def get_pager_info(Model, filter_query, url, id, per_items=20):
    ...
    if url == '/api/project_list/':
        ...
        obj = obj.filter(name__contains=name).values('id', 'name', 'kwargs', 'enabled', 'date_changed') \
            if name is not '' else obj.filter(~Q(name__contains='celery.backend_cleanup')).values('id', 'name', 'kwargs', 'enabled', 'date_changed', 'description')
        ...
        if url == '/api/test_list_dubbo/':
            obj = obj.filter(type__exact=1, dubbo_is=1)
    ...
    if total != 0:
        ...
        page_list = customer_pager(url, id, page_info.total_page, total)
    return page_list, info, sum

def add_cases(Model,num):
    data=Model.objects.get(id=num)
    type_1=data.type
    name=data.name
    ...
    belong_module_id=data.belong_module_id
    create_time=data.create_time
    update_time=data.update_time

    request=ast.literal_eval(request)
    req=copy.deepcopy(request)
    if request['test']['request']['method']=='POST':
        try:
            request['test']['request']['json']
            body=request['test']['request']['json']
            flag='json'
        except:
            body=request['test']['request']['data']
            flag='data'

    elif request['test']['request']['method']=='GET':
        body = request['test']['request']['params']
        flag='params'
    ...
    for index in range(len):
        if flag=='params':
            req['test']['request']['params'].pop(keys_list[index])
        elif flag=='data':
            req['test']['request']['data'].pop(keys_list[index])
        else:
            req['test']['request']['json'].pop(keys_list[index])
        dic={'type':type_1,'name':'%s缺少%s'%(name,keys_list[index]),'belong_project':belong_project,'include':include,'request':req,
             'author':author,'belong_module_id':belong_module_id,'create_time':create_time,'update_time':update_time}
